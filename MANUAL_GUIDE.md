...
3. Проверьте порядок блоков:
   ...
   - год,
   - цена.
4. Проверьте длинные значения в Airtable, например длинное название. PDF не должен падать.
5. Проверьте запись без изображения. PDF должен сформироваться с placeholder.
