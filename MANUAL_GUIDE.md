...
Ожидаемый результат: тесты завершаются успешно, локальный e2e печатает сценарии, в папке `output` создается `sample_artwork_2.pdf`.
...
Ожидаемый ответ: бот говорит, что нужно прислать номер строки / числовой ID или фамилию автора.
...
Ожидаемый ответ: краткая инструкция по поиску по номеру и по автору.
...
Отправьте фамилию автора, которая есть в Airtable:
...
1. Бот отвечает, сколько работ найдено.
2. Бот отправляет один общий PDF.
3. Каждая найденная работа находится на отдельной странице PDF.
4. Отдельные карточки в чат не отправляются.

### Проверка неизвестного автора

Отправьте фамилию, которой нет в Airtable:

```text
Сидоров
```

Ожидаемый ответ: бот сообщает, что работы по такому автору не найдены.

### Проверка слишком короткого запроса по автору
...
Ожидаемый ответ: бот просит прислать минимум 2 символа.
...
- [ ] Фамилия автора возвращает один общий PDF.
- [ ] Неизвестный автор возвращает понятное сообщение.
- [ ] Слишком короткий текстовый ввод возвращает подсказку.
