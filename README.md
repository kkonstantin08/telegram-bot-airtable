...
Telegram-бот для коллекции современного искусства. Пользователь отправляет боту только номер строки / стабильный числовой ID записи, бот находит ровно одну запись в Airtable, отправляет карточку объекта в Telegram и прикладывает PDF A4 по единому шаблону.

## Что делает проект

- Команда `/start` кратко объясняет сценарий.
- Команда `/help` показывает инструкцию.
- Основной сценарий: пользователь отправляет число, например `12`.
- Бот ищет запись в Airtable строго по числовому полю, заданному в `AIRTABLE_ROW_ID_FIELD`.
- Если запись найдена, бот отправляет изображение с подписью или текстовую карточку без изображения.
- Затем бот формирует и отправляет PDF-карточку A4.
- Если записей с одним row ID несколько, бот не выбирает одну автоматически и сообщает об ошибке данных.

В проекте намеренно нет поиска по названию, частичных совпадений, fuzzy search, inline keyboard и списка вариантов.
