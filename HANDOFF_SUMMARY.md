...
- Реализован единственный сценарий поиска: пользователь отправляет положительное число, бот ищет запись строго по стабильному числовому полю `AIRTABLE_ROW_ID_FIELD`.
- Реализована отправка Telegram-карточки: изображение плюс подпись или текстовая карточка при отсутствии изображения.
- Реализована генерация PDF A4 по единому шаблону.
...
- `pytest`: 13 тестов прошли успешно.
...
- Проверить PDF на нескольких длинных названиях и ценах.
